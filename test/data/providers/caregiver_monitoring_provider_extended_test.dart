...
void main() {
  ...
  group('patientDailyAdherenceProvider — SC-CAR-003', () {
    test('returns 100.0 when today has no reminders', () async {
      final container = ProviderContainer(
        overrides: [
          patientRemindersProvider('patient-1').overrideWith(
            (ref) => Stream.value([]),
          ),
        ],
      );
      ...
      final result =
          await container.read(patientDailyAdherenceProvider('patient-1').future);
  ...
  group('patientDailyAdherenceProvider — SC-CAR-005', () {
    test('excludes yesterday reminders; returns 100.0 for 1 taken today', () async {
      final today = DateTime.now();
  ...
  group('patientMedicationStatusProvider — SC-CAR-006', () {
    test('returns correct status and variant for taken and missed', () async {
      ...
      final reminders = [
        _makeReminder('r1', 'med-a',
            DateTime(today.year, today.month, today.day, 8, 0),
            ReminderStatus.taken),
        _makeReminder('r2', 'med-b',
            DateTime(today.year, today.month, today.day, 8, 0),
            ReminderStatus.missed),
      ];
      ...
      final container = ProviderContainer(
        overrides: [
          patientMedicationsProvider('patient-1').overrideWith(
            (ref) => Stream.value([medA, medB]),
          ),
          patientRemindersProvider('patient-1').overrideWith(
            (ref) => Stream.value(reminders),
          ),
      ...
      final result = await container
          .read(patientMedicationStatusProvider('patient-1').future);
  ...
  group('patientMedicationStatusProvider — SC-CAR-007', () {
    test('defaults to pending variant when no today reminders for medication',
        () async {
      final medA = _makeMed('med-a', 'Med A');
  ...
  group('patientMedicationStatusProvider — SC-CAR-008', () {
    test('uses most recent reminder when multiple exist for same medication',
        () async {
      final today = DateTime.now();
      final medA = _makeMed('med-a', 'Med A');
  ...
  // ---------------------------------------------------------------------------
  group('_reminderStatusToVariant — SC-CAR-009 (via patientMedicationStatus)', () {
    Future<MpBadgeVariant> variantFor(ReminderStatus status) async {
      ...
      final container = ProviderContainer(
        overrides: [
          patientMedicationsProvider('p').overrideWith(
            (ref) => Stream.value([med]),
          ),
          patientRemindersProvider('p').overrideWith(
      ...
      final result =
          await container.read(patientMedicationStatusProvider('p').future);
      return result[0]['variant'] as MpBadgeVariant;
    }

    test('taken -> MpBadgeVariant.taken', () async {
      expect(await variantFor(ReminderStatus.taken), equals(MpBadgeVariant.taken));
    });

    test('missed -> MpBadgeVariant.missed', () async {
      expect(
          await variantFor(ReminderStatus.missed), equals(MpBadgeVariant.missed));
    });

    test('snoozed -> MpBadgeVariant.snoozed', () async {
      expect(await variantFor(ReminderStatus.snoozed),
          equals(MpBadgeVariant.snoozed));
    });

    test('skipped -> MpBadgeVariant.missed (same as missed)', () async {
      expect(await variantFor(ReminderStatus.skipped),
          equals(MpBadgeVariant.missed));
    });

    test('pending -> MpBadgeVariant.upcoming', () async {
      expect(await variantFor(ReminderStatus.pending),

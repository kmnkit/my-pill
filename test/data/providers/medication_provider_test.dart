...
void main() {
  ...
  group('MedicationList provider', () {
    ...
    test('updateMedication saves updated medication and invalidates provider',
        () async {
      final med = _makeMedication('med-1', 'Aspirin');
    ...
    test('deleteMedication calls cascade delete and invalidates provider',
        () async {
      final med = _makeMedication('med-1', 'Aspirin');
      ...
      when(mockStorage.deleteRemindersForMedication(any))
          .thenAnswer((_) async {});
      when(mockStorage.deleteAdherenceRecordsForMedication(any))
          .thenAnswer((_) async {});
      when(mockStorage.getSchedulesForMedication(any))
          .thenAnswer((_) async => []);
  ...
  group('medication provider (single)', () {
    test('returns medication when found', () async {
      ...
      final container = makeContainer();
      final result =
          await container.read(medicationProvider('med-1').future);
    ...
    test('returns null when medication not found', () async {
      when(mockStorage.getMedication('non-existent'))
          .thenAnswer((_) async => null);

      final container = makeContainer();
      final result =
          await container.read(medicationProvider('non-existent').future);

...
List<dynamic> _buildOverrides({
  ...
  return [
    ...
    if (streamError)
      ...
      authStateProvider.overrideWith(
        (ref) => Stream.value(streamNull ? null : user),
      ),
  ];
}
...
void main() {
  group('AccountSection', () {
    ...
    testWidgets('falls back to email prefix when no display name', (
      ...
      final user = _FakeUser(displayName: null, email: 'john@example.com');
      await tester.pumpWidget(
        createTestableWidget(
          const AccountSection(),
          overrides: _buildOverrides(
    ...
    testWidgets('shows "Email hidden" for Apple private relay email', (
      ...
      expect(find.text('abc123@privaterelay.appleid.com'), findsNothing);
    });
    ...
    testWidgets('shows "No email" when user has no email', (tester) async {
      final user = _FakeUser(displayName: 'No Email User', email: null);
      await tester.pumpWidget(
        createTestableWidget(
          const AccountSection(),
          overrides: _buildOverrides(
    ...
    // =========================================================================
    testWidgets('anonymous user shows sign in to sync subtitle', (
      tester,
    ) async {
      final anonUser = _FakeUser(isAnonymous: true);
      await tester.pumpWidget(
    ...
    testWidgets(
      ...
      (tester) async {
        final user = _FakeUser(displayName: null, email: null);
        await tester.pumpWidget(
          createTestableWidget(
            const AccountSection(),
            overrides: _buildOverrides(
    ...
    // =========================================================================
    testWidgets('empty display name shows question-mark initial', (
      tester,
    ) async {
      final user = _FakeUser(displayName: '', email: null);
      await tester.pumpWidget(
        createTestableWidget(
          const AccountSection(),
          overrides: _buildOverrides(
    ...
    // =========================================================================
    testWidgets(
      'profile name empty string falls back to Firebase displayName',
      (tester) async {
        final user = _FakeUser(

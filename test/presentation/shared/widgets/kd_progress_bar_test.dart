  ...
  group('KdProgressBar', () {
    testWidgets('renders LinearProgressIndicator', (tester) async {
      await tester.pumpWidget(
        createTestableWidget(const KdProgressBar(current: 3, total: 10)),
      );
    ...
    testWidgets('label hidden when showLabel is false', (tester) async {
      await tester.pumpWidget(
        createTestableWidget(
          const KdProgressBar(current: 3, total: 10, showLabel: false),
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('semantic label includes current, total and percent', (
      ...
      await tester.pumpWidget(
        createTestableWidget(const KdProgressBar(current: 5, total: 10)),
      );
      await tester.pumpAndSettle();

      // The Semantics widget wraps with a value of '50%'
      final semantics = tester.getSemantics(find.byType(KdProgressBar));
      expect(semantics.value, '50%');
    });

    testWidgets('handles zero total gracefully', (tester) async {
      await tester.pumpWidget(
        createTestableWidget(const KdProgressBar(current: 0, total: 0)),
      );
      await tester.pumpAndSettle();

      expect(find.byType(KdProgressBar), findsOneWidget);
    });

    testWidgets('renders in Japanese locale without error', (tester) async {
      await tester.pumpWidget(
        createTestableWidgetJa(const KdProgressBar(current: 2, total: 5)),
      );
      await tester.pumpAndSettle();
